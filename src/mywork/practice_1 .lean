...
Proof: The conjecture that z = w is true is proven by using the axiom of symmetry on 
the assumption that w=z. 
-/
...
type of X.) 
The introduction rule for ∀ is that it applies to every 
single example of that object
-/
